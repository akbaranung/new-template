...
class M_coa extends CI_Model
{
	...
	public function getCoaBB($no_coa)
	{
		if ($no_coa == "ALL") {
			...
			return $this->cb->where(['no_bb' => $no_coa, 'id_cabang' => $this->session->userdata('kode_cabang')])->get('v_coabb_all')->row_array();
		}
	}
	...
	public function list_coa_bb_paginate($limit, $from, $keyword, $cabang)
	{
		// $this->cb;
		// $this->apply_cabang_filter();
	...
	{
		return $this->cb->where('periode', $periode)
			->where('id_cabang', $this->session->userdata('kode_cabang'))
			->update('saldo_awal', $data);
	}
	...
	public function getNeracaConsolByDate($table, $posisi, $tanggal_akhir, $id_company)
	{
		...
		if ($posisi == "AKTIVA") {
            ...
            FROM 
                (SELECT DISTINCT no_sbb, nama_perkiraan, posisi 
                 FROM v_coa_all 
                 WHERE id_company = '$id_company' 
                 AND table_source = '$table' 
                 AND posisi = '$posisi') coa
            LEFT JOIN jurnal_neraca jn 
                ON (coa.no_sbb = jn.akun_debit OR coa.no_sbb = jn.akun_kredit)
                AND jn.id_company = '$id_company'
                AND jn.tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir'
            GROUP BY 
                coa.no_sbb, coa.nama_perkiraan, coa.posisi
            ORDER BY 
                coa.no_sbb ASC
	...
	public function getCoaReportAnnually($no_coa, $year)
	{
		...
		$this->cb->where('id_cabang', $this->session->userdata('kode_cabang')); // ← pindah ke luar
		$this->cb->where('id_company', $id_company);                            // ← pindah ke luar
		$this->cb->group_start();
		$this->cb->where('akun_debit', $no_coa);
		$this->cb->or_where('akun_kredit', $no_coa);
		$this->cb->group_end();
