...
class M_invoice extends CI_Model
{
	...
	// part sales
	public function list_sales($limit, $from, $keyword = null, $keyword_opt = 'all')
	{
		$kode_cabang = $this->session->userdata('kode_cabang');

		// pastikan query fresh
		$query = $this->cb->from('sales');

		// filtering keyword
		if ($keyword && $keyword_opt !== 'all') {
			$query->like($keyword_opt, $keyword);
		} else if ($keyword) {
			$query->group_start()
				->like('no_smu', $keyword)
				->or_like('airline_name', $keyword)
				->or_like('jenis_barang', $keyword)
				->group_end();
		}

		// wajib: selalu filter cabang
		$query->where('id_cabang', $kode_cabang);
		...
		foreach ($sales as &$sale) {
			$sale['created_by_name'] = $user_map[$sale['upload_by']] ?? null;
			// $sale['updated_by_name'] = $user_map[$sale['updated_by']] ?? null;
		}
	...
	public function sales_count($keyword = null, $keyword_opt = 'all')
	{
		...
		$builder = $this->cb->from('sales');

		if ($keyword && $keyword_opt !== 'all') {
			$builder->like($keyword_opt, $keyword);
		} else if ($keyword) {
			$builder->group_start()
				->like('no_smu', $keyword)
				->or_like('airline_name', $keyword)
		...
		$builder->where('id_cabang', $kode_cabang);

		return $builder->count_all_results();
	}

	public function insert_batch_sales($data)
	{
		return $this->cb->insert_batch('invoice_sales_detail', $data); // sesuaikan nama tabel
	}

	public function get_invoice_sales_details($id_invoice)
	{
		$this->cb->select('isd.*, s.no_smu, s.tanggal_terbang, s.flight_number, s.origin, s.destination, s.koli as sales_koli, s.gross, s.chargeable_weight');
		$this->cb->from('invoice_sales_detail isd');
		$this->cb->join('sales s', 's.Id = isd.id_sales', 'left');
		$this->cb->where('isd.id_invoice', $id_invoice);
