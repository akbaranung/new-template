...
class Tabungan extends CI_Controller
{
  ...
  public function ajax_list()
  {
    ...
    foreach ($list as $cat) {
      ...
      $row[] = $cat->pos_rate;
      $row[] = $cat->nolsp;
      $delete_url = base_url('tabungan/delete/' . $cat->no_cib);

      $row[] = '<a class="btn btn-warning m-1" href="' . base_url('tabungan/edit/' . $cat->no_cib) . '">Edit</a> <a class="btn btn-danger m-1" 
  ...
  public function add_tabungan()
  {
    ...
    $data['pages_script'] = 'script/tabungan/s_tabungan';
    $data['user'] = $this->db->get_where('users', ['nip' => $nip])->row_array();
    $data['pages'] = 'pages/tabungan/v_tabungan_form';
    $data['menus'] = $this->M_menu->get_accessible_menus($nip);
